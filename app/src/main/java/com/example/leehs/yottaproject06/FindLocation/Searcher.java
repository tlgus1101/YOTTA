...
public class Searcher {
		...
		protected Void doInBackground(String... urls) {
			...
			if (onFinishSearchListener != null) {
				if (itemList == null ) {
					...
					Log.d("sendcase : ", sendCase);

					switch (sendCase) {
						case SINGLE:
							if(itemList.size() != 0) {
								Item item = itemList.get(0);
								onFinishSearchListener.onSuccessSingle(item);
								break;
							}else{
								break;
							}
						...
						case MULTI:
							for(int i = 0; i < itemList.size(); ++i) {
								Log.d("itemList 확인", itemList.get(i).title);
							}
	...
	public void searchKeywordSingle(Context applicationContext, String query, double latitude, double longitude, int radius, int page, String apikey, OnFinishSearchListener onFinishSearchListener) {
